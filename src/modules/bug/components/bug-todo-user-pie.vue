  ...
  methods: {
    ...
    calcData () {
      ...
      for (let i = 0, len = this.dataList.length; i < len; i++) {
        bug = this.dataList[i]
        if (data[bug.curUserId]) {
          data[bug.curUserId] += 1
        } else {
          data[bug.curUserId] = 1
        }
      }
      for (let key in data) {
        name = this.$utils.userFormatter(key, this.userList)
    ...
    drawChart () {
      this.chart = echarts.init(document.getElementById('bug-todo-user-pie'))
